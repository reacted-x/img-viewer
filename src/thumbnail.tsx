...
const Thumbnail: React.FunctionComponent<IThumbnailProps> = ({
  ...
  // 获取缩略图外框的样式，主要就是宽和高
  const thumbnailBoxStyle = useMemo(() => {
  ...
  const thumbnailImgStyle = useMemo(() => {
    ...
    let { height: boxHeight, width: boxWidth } = thumbnailBoxStyle;
  ...
  }, [realSize, thumbnailBoxStyle, offset, viewportSize]);

  return (
    <SCThumbnailBox style={thumbnailBoxStyle}>
      <SCThumbnailImg style={thumbnailImgStyle} rotate={rotate} src={bgUrl} />
      <SCThumbnailViewTip style={tipStyle} />
    </SCThumbnailBox>
  );
};
